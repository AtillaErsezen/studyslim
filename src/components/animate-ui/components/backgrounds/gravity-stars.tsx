...
function GravityStarsBackground({
  ...
  const readColor = React.useCallback(() => {
    ...
    // Prefer the brand green variable '--lime' if defined, fallback to computed color
    const brandLime = cs.getPropertyValue('--primary')?.trim();
    if (brandLime) return brandLime;
    return cs.color || '#6366f1';
  }, []);
  ...
  const drawStars = React.useCallback(
    (ctx: CanvasRenderingContext2D) => {
      ...
      const color = cs?.getPropertyValue('--lime')?.trim() || readColor();
      const glowColor = cs?.getPropertyValue('--lime-glow')?.trim() || color;
